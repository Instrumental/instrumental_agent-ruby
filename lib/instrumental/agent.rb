  ...
  class Agent
    ...
    def test_connection
      begin
        ...
        # no-op
        @logger.error "test connection failed with a wait exception in #{Process.pid}"
      rescue Exception => e
        @logger.error "test connection failed with an unknown exception #{e.class} in #{Process.pid}"
      end
    end
